...
//     const [options, setOptions] = useState([]);
//     const [newOption, setNewOption] = useState('');
//     const [type, setType] = useState('select'); // Состояние для хранения выбранного типа
...
//         const validOptions = options.filter((option) => option.trim() !== '');
//         if (label.trim() !== '' && validOptions.length > 0) {
//             onAddField({ label, type, options: validOptions, value: '' }); // Передаем тип поля
//             setLabel('');
//             setOptions([]);
//             setType('select'); // Сбрасываем тип поля к значению по умолчанию
//         }
//     };
...
//      const validOptions = options.filter((option) => option.trim() !== '');
//     if (label.trim() !== '' && validOptions.length > 0) {
//       onAddField({ label, type: 'select', options: validOptions, value: '' });
//       setLabel('');
//       setOptions([]);
//     }
//   };
...
//               required
//             />
//              <button onClick={() => handleDeleteOption(index)}>Удалить вариант</button>
//           </div>
//          ))}
//             <input
//              type="text"
